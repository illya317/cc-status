...
支持 **Claude 官方模型** 和 **第三方模型**（DeepSeek、Kimi、MiniMax、GLM、GPT-5.5），实时 token 追踪与计费。
...
- **平台余额** — 实时 DeepSeek 余额（¥）、Kimi 配额、MiniMax 配额、GLM 配额
- **会话费用** — 从 transcript 解析 token 用量计算（含子 Agent token）
- **Agent 监控** — 显示运行中/已完成的 Agent 子任务及耗时（compact 第2行，split 第3行）
